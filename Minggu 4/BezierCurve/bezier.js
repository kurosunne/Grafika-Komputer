...
const svg = document.querySelector("path").getAttribute('d');
...
function main() {
  ...
  const texInfo = loadImageAndCreateTextureInfo("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTgyjmZoKFIuEcQy40PopAsanDbiUC2r_WuKg&usqp=CAU", render);
  ...
  function render() {
    ...
    const fieldOfViewRadians = Math.PI * .1;
    const aspect = gl.canvas.clientWidth / gl.canvas.clientHeight;
    projectionMatrix =
        m4.perspective(fieldOfViewRadians, aspect, 1, 2000);

    const midY = lerp(extents.min[1], extents.max[1], .5);
    const sizeToFitOnScreen = (extents.max[1] - extents.min[1]) * .7;
    const distance = sizeToFitOnScreen / Math.tan(fieldOfViewRadians * .6);
  ...
  function rotateCamera(e) {
    if (moving) {
      ...
      const size = [2 / gl.canvas.width, 2 / gl.canvas.height];
      const delta = v2.mult(v2.sub(lastPos, pos), size);

      worldMatrix = m4.multiply(m4.xRotation(delta[1] * 3), worldMatrix);
      worldMatrix = m4.multiply(m4.yRotation(delta[0] * 3), worldMatrix);
